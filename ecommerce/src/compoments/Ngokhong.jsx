...
export default function Ngokhong(){
    const navigate = useNavigate();
    const [products, setProducts] = useState([]);
    useEffect(()=>{
        const fetchProduct = async ()=>{
            try{
                const response = await axios.get("https://localhost:7048/api/Products/Hottrend/black%20wukong");
    ...
    const title = "Black WuKong";
    const handleClick = (title) => {
        const encodedTitle = encodeURIComponent(title);
        navigate(`/products/${encodedTitle}`);
    };
    return(<div className={styles.sanpham}>
    <div className={styles.Onepice}><button><h1>Black WuKong</h1></button></div>
